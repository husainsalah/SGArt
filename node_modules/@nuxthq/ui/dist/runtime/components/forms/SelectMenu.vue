...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const ui = computed(() => defu({}, props.ui, appConfig2.ui.selectMenu));
    const uiSelect = computed(() => defu({}, props.uiSelect, appConfig2.ui.select));
    const popper = computed(() => defu({}, props.popper, ui.value.popper));
    const [trigger, container] = usePopper(popper.value);
    const query = ref("");
    const searchInput = ref();
    const selectMenuClass = computed(() => {
      const variant = uiSelect.value.color?.[props.color]?.[props.variant] || uiSelect.value.variant[props.variant];
      return classNames(
        uiSelect.value.base,
    ...
    const leadingIconClass = computed(() => {
      return classNames(
        ...
        appConfig2.ui.colors.includes(props.color) && uiSelect.value.icon.color.replaceAll("{color}", props.color),
